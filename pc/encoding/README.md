...
来自: https://www.zhihu.com/answer/676684648
...
![bg](http://www.ruanyifeng.com/blog/2007/10/bg2007102801-thumb.jpg)
...
主要参考:  https://www.ruanyifeng.com/blog/2010/02/url_encoding.html
...
中文经由编码后使用, 而编码的方式由浏览器决定.

通常有几种情况:

1. 网址路径中包含汉字: UTF-8 转码
2. 查询字符串包含汉字: 操作系统的默认编码
3. Get方法生成的URL包含汉字: 网页的编码
4. Ajax调用的URL包含汉字: IE总是采用GB2312编码（操作系统的默认编码），而Firefox总是采用utf-8编码。这就是我们的结论4

> 有没有办法，能够保证客户端只用一种编码方法向服务器发出请求？

使用Javascript先对URL编码，然后再向服务器提交，不要给浏览器插手的机会。因为Javascript的输出总是一致的，所以就保证了服务器得到的数据是格式统一的--Unicode

- `escape()`不能直接用于URL编码，它的真正作用是返回一个字符的Unicode编码值。比如"春节"的返回结果是%u6625%u8282，也就是说在Unicode字符集中，"春"是第6625个（十六进制）字符，"节"是第8282个（十六进制）字符。它的具体规则是，除了ASCII字母、数字、标点符号"@ * _ + - . /"以外，对其他所有字符进行编码。在`\u0000`到`\u00ff`之间的符号被转成%xx的形式，其余符号被转成`%uxxxx`的形式。对应的解码函数是`unescape()`
- `encodeURI()`: 着眼于对整个URL进行编码，因此除了常见的符号以外，对其他一些在网址中有特殊含义的符号"; / ? : @ & = + $ , #"，也不进行编码。编码后，它输出符号的utf-8形式，并且在每个字节前加上%。解码函数是`decodeURI()`。需要注意的是，它不对单引号'编码。
- `encodeURIComponent()`:它用于对URL的组成部分进行个别编码，而不用于对整个URL进行编码。"; / ? : @ & = + $ , #"，这些在encodeURI()中不被编码的符号，在encodeURIComponent()中统统会被编码。至于具体的编码方法，两者是一样。

## MD5

- windows方法：win键+r键输入cmd调出命令行
  MD5： `certutil -hashfile 文件名  MD5`
  查看 SHA1： `certutil -hashfile 文件名  SHA1 `
  查看SHA256： `certutil -hashfile 文件名  SHA256`
- LINUX方法，进入文件目录`md5sum 文件名`
