...
2. Verilog 的优点：
    ...
    2. 能够在多个层次上对所设计的系统进行描述：开关级、门级、寄存器传输级、行为级。
    3. 不对设计的规模施加任何限制。
    4. 有混合建模能力，即在一个设计中，各个模块可在不同层次上建模和描述。
    5. 灵活多样的电路描述风格：行为描述、结构描述、数据流描述。
    6. 其行为描述语句（条件、赋值、循环）类似于高级语言，易学易用。
    7. 内置各种基本逻辑门，适合门级建模。内置各种开关级元件，适合开关级建模。可灵活创建用户定义原语（UDP）。
    8. 可通过编程语言接口（PLI）调用C语言编写的各种函数。
