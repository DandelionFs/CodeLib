...
1. 文件读写使用
   项目中的目录的读取.
   [如何利用C/C++逐行读取txt文件中的字符串(可以顺便实现文本文件的复制)](https://blog.csdn.net/stpeace/article/details/12404925)
2. string 与 字符数组的使用要统一.
3. linux 下 getline 获取的 多一位'\r'

## Huffman Coding | 最优二叉树 | 哈夫曼编码 | 赫夫曼编码

> 使编码之后的字符串的平均长度、期望值降低，从而达到无损压缩数据的目的。

- 常见的编码表: 在英文中，e的出现几率最高，而z的出现概率则最低。
