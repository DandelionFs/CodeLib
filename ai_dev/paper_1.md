...
- DBC算法通过计算用户密度高的区域以决定最适合分配物理云服务器的地理位置。DBC算法考虑了负载均衡，在给定用户位置的情况下，最小化服务响应时间方面更有效。

在移动边缘计算中服务迁移问题上强化学习也起了重要的作用。Ksentini提出了基于一维（1—D）移动模式的马尔可夫决策过程，通过权衡迁移成本和用户感知质量来决定策略[40），之后， Bokani进一步提出了车载环境下的MDP方法，由于网络带宽的变化，需要基于HTTP的自适应流优化控制策略[1）。然而，这些研究主要考虑具有特定成本函数的一维（1—D）移动模式。因此， Wang进一步考虑二维（2—D）移动模式，其中二维（2—D）移动模式的MDP问题由一维（1—D）移动模式的基于距离的MDP近似求解（42），然而，它仅仅考虑了单个用户的情况，没有研究多用户的多维移动模式。在本文研究的问题中，考虑的不是单用户单服务的情况，而是一个服务被多个移动用户共享的场景。
...
强化学习在移动边缘计算场景下服务迁移问题上也起了重要的作用，服务部署问题上目前还没有很多的研究，由于服务部署问题本质上也是一个决策优化问题，所以可以使用强化学习的技术来解决。下面将对强化学习的基本原理以及马尔科夫决策过程进行介绍。
