...
St StuAcc[Studentnum+ErrStudentnum]; //学生用户组
int StuFlag;
Te TeaAcc[Teachernum+ErrTeachernum]; //老师用户组
int TeaFlag;
...
void QuitSystem(){//保存文件到目录下
    ...
    outfile.open("./data/TeaInfo.out");
    for(int i=0;i<Teachernum+ErrTeachernum;i++){
        if(!TeaAcc[i].SuDel)
            outfile<<TeaAcc[i].name<<"\t"<<TeaAcc[i].ID<<'\t'<<TeaAcc[i].PassW;
        outfile<<endl;
    }
    outfile.close();
...
void PrintLoginin(){
    ...
    if(order==1){
        int oorder;
L1:     cout<<"输入名字\n";
        cin>>StuAcc[Studentnum+StuFlag].name;
        if(IsStLogin(StuAcc[Studentnum+StuFlag].name)!=-1){
            cout<<"用户已注册";
            goto L1;
        }
L2:     cout<<"输入学号\n";
        cin>>StuAcc[Studentnum+StuFlag].ID;
        if(IsStLoginID(StuAcc[Studentnum+StuFlag].ID)!=-1){
            cout<<"用户已注册";
            goto L2;
        }
        
        cout<<"输入密码\n";
        StuAcc[Studentnum+StuFlag].PassW=InputNameCheck();
        cout<<"选择班级\n";
        SuShowGradeInfo();
        cin>>oorder;
        oorder=FoundRealGrade(oorder);
        StuAcc[StuFlag+Studentnum].Grade=oorder;
        StuFlag++;
        system("reset");
        cout<<"注册成功.\n";
        PAUSE();
    ...
    if(order==2){
L3:     cout<<"输入名字\n";
        cin>>TeaAcc[Cnum+TeaFlag].name;
        if(IsTeaLogin(TeaAcc[Cnum+TeaFlag].name)!=-1){
            cout<<"用户已注册\n";
            goto L3;
        }
L4:     cout<<"输入学号\n";
        cin>>TeaAcc[Cnum+TeaFlag].ID;
        if(IsTeaLoginID(TeaAcc[Cnum+TeaFlag].ID)!=-1){
            cout<<"用户已注册\n";
            goto L4;
        }
        cout<<"输入密码\n";
        TeaAcc[Cnum+TeaFlag].PassW=InputNameCheck();
        TeaFlag++;
        system("reset");
        cout<<"老师注册成功.\n";
        PAUSE();
...
int IsTeaLogin(string a){
    int flag =-1;
    for(int i=0;i<Teachernum+TeaFlag;i++){
        if(TeaAcc[i].name==a){
            flag=i;
            return flag;
        }
...
void ChooseCourse(int a){//进行已有的课程的选择
    ...
    } while (num>Cnum+Flag_ErrCourse);//输入一定要用循环
...
void SuShowCourseAStudent(){//课程列表以及学生的信息
    ...
    for(jnum=0; jnum<Cnum; jnum++){
        if(!SuDeleteCourse[jnum]){
            cout<<"["<<step++<<"]\t"<<course[jnum]<<'\t';
            for(int j=0;j<Studentnum+StuFlag;j++){
                if(!StuAcc[j].SuDel && StuAcc[j].course[jnum])
                    cout<<StuAcc[j].name<<" ";
            }
        }
        else continue;
    ...
    for(int i=0;i<Flag_ErrCourse;i++){
        if(!SuDeleteCourse[jnum+i]){
            cout<<"["<<step++<<"]\t"<<ErrCourse[i]<<'\t';
            for(int j=0;j<Studentnum+StuFlag;j++){
                if(!StuAcc[j].SuDel && StuAcc[j].course[i])
                    cout<<StuAcc[j].name<<" ";
            }
        }
        else continue;
...
void SuShowAllStINfo(){ //展示所有学生的选课信息
    ...
    for(int i=0;i<Studentnum+StuFlag;i++){
        if(!StuAcc[i].SuDel)
            cout<<StuAcc[i].name<<'\t'<<StuAcc[i].ID<<'\t'<<Gradecourse[StuAcc[i].Grade]<<'\t'<<StuAcc[i].CCourse<<'\t'<<'\t';
        else continue;
        for(int j=0 ;j<Cnum+Flag_ErrCourse;j++)
            if(StuAcc[i].course[j] && !SuDeleteCourse[j]){
                if(j<Cnum)
